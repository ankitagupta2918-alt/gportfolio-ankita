...
// Scroll progress + ambient mouse light
const scrollProgress=document.getElementById('scrollProgress');
const mouseGlow=document.getElementById('mouseGlow');
function updateScrollProgress(){const h=document.documentElement.scrollHeight-window.innerHeight;scrollProgress.style.width=(h>0?(scrollY/h)*100:0)+'%'}
window.addEventListener('scroll',updateScrollProgress,{passive:true});updateScrollProgress();
window.addEventListener('mousemove',e=>{if(mouseGlow){mouseGlow.style.left=e.clientX+'px';mouseGlow.style.top=e.clientY+'px'}});
